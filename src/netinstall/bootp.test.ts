...
function buildTestDiscover(opts: { transactionId: number; clientMac: string }): Uint8Array {
...
describe("BootpServer", () => {
  ...
  it("emits discover and offer on valid DHCP DISCOVER", async () => {
    ...
    const discoverPkt = buildTestDiscover({
  ...
  it("emits error when send fails", async () => {
    mockDgramSocket.send.mockImplementation(
      (_data: unknown, _port: unknown, _addr: unknown, cb?: (e: Error | null) => void) =>
  ...
  it("rejects listen() when setBroadcast throws", async () => {
    mockDgramSocket.setBroadcast.mockImplementationOnce(() => {
