...
vi.mock("./bootp", async (importOriginal) => {
  ...
  return {
    ...actual,
    BootpServer: vi.fn(function BootpServerMock(): BootpServerType {
      return mockBootp as unknown as BootpServerType;
    }),
  };
});
...
vi.mock("./tftp", async (importOriginal) => {
  ...
  return {
    ...actual,
    TftpServer: vi.fn(function TftpServerMock(): TftpServerType {
      return mockTftp as unknown as TftpServerType;
    }),
  };
});
...
function makeSession(
  overrides: Partial<ConstructorParameters<typeof NetinstallSession>[0]> = {}
): NetinstallSession {
...
function completeTransfer(imageSize = 100): void {
  ...
  mockTftp._emit("progress", {
    blockNumber: 1,
    bytesTransferred: imageSize,
