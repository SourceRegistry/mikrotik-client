...
export class NetinstallSession extends EventEmitter {
    ...
    async start(opts?: { signal?: AbortSignal }): Promise<void> {
        if (this.state !== "idle") {
            throw new Error(`NetinstallSession.start() called in state "${this.state}" — create a new instance per installation`);
        }
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            bootp.on("discover", (packet: BootpPacket) => {
                this.emit("progress", { kind: "offering", deviceMac: packet.clientMac } satisfies NetinstallProgressEvent);
            });
            ...
            tftp.on("request", ({ filename, clientAddress }: { filename: string; clientAddress: string }) => {
                this.state = "running";
                this.emit("progress", { kind: "tftp_start", filename, clientAddress } satisfies NetinstallProgressEvent);
            });
            ...
            tftp.on("complete", () => {
                ...
                this.emit("progress", { kind: "tftp_complete", totalBytes } satisfies NetinstallProgressEvent);
            ...
            Promise.all([
                bootp.listen(this.bootpPort),
                tftp.listen(this.tftpPort),
            ]).catch((err: unknown) => {
                clearTimeout(timeoutHandle);
                signal?.removeEventListener("abort", onAbort);
