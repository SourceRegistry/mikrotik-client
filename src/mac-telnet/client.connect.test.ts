...
function makeMockSocket(): MockSocket {
    ...
    const sock: MockSocket = {
        ...
        bind: vi.fn(() => { Promise.resolve().then(() => sock._emit("listening")); return sock; }),
        close: vi.fn(),
        removeAllListeners: vi.fn(() => { for (const k of Object.keys(handlers)) handlers[k] = []; }),
    };
    return sock;
}
...
vi.mock("node:crypto", () => ({
    ...
    createHash: vi.fn(() => ({ update: vi.fn().mockReturnThis(), digest: vi.fn(() => new Uint8Array(16)) })),
    createHmac: vi.fn(() => ({ update: vi.fn().mockReturnThis(), digest: vi.fn(() => new Uint8Array(32)) })),
}));
vi.mock("./interfaces", () => ({
    validateInterface: vi.fn(() => ({ name: "en0", mac: "aa:bb:cc:dd:ee:ff", isUp: true, addresses: [] })),
...
vi.mock("./packet", async (importOriginal) => {
    ...
    return {
        ...actual,
        decodePacket: vi.fn(() => ({ ptype: "start" as const, counter: 0, srcMac: new Uint8Array(6).fill(0), dstMac: new Uint8Array(6).fill(0), sessionKey: 0 })),
...
beforeEach(async () => {
    ...
    vi.mocked(decodePacket).mockReturnValue({ ptype: "start" as const, counter: 0, srcMac: new Uint8Array(6), dstMac: new Uint8Array(6), sessionKey: 0 });
    vi.mocked(decodeControlPackets).mockReturnValue({ controlPackets: [], rawData: new Uint8Array(0) });
    const dgramMod = await import("node:dgram");
    vi.mocked(dgramMod.default.createSocket).mockReturnValue(mockSocket as unknown as ReturnType<typeof dgramMod.default.createSocket>);
});
...
type ClientInternals = {
    ...
    pendingCommands: Map<number, { resolve: (r: unknown) => void; reject: (e: Error) => void; timer: ReturnType<typeof setTimeout> }>;
...
describe("MacTelnetClient handleMessage", () => {
    it("emits error on malformed packet", async () => {
        ...
        decodePacket.mockImplementationOnce(() => { throw new Error("bad packet"); });
        const errors: Error[] = [];
    ...
    it("handles END packet → state becomes disconnected", async () => {
        const client = await forceConnected();
        const { decodePacket } = vi.mocked(await import("./packet"));
        decodePacket.mockReturnValueOnce({ ptype: "end" as const, counter: 0, srcMac: new Uint8Array(6), dstMac: new Uint8Array(6), sessionKey: 0 });
    ...
    it("handles ACK packet silently", async () => {
        const client = await forceConnected();
        const { decodePacket } = vi.mocked(await import("./packet"));
        decodePacket.mockReturnValueOnce({ ptype: "ack" as const, counter: 0, srcMac: new Uint8Array(6), dstMac: new Uint8Array(6), sessionKey: 0 });
    ...
    it("emits data event when connected and rawData present", async () => {
        ...
        decodePacket.mockReturnValueOnce({ ptype: "data" as const, counter: 42, srcMac: new Uint8Array(6), dstMac: new Uint8Array(6), sessionKey: 0 });
        decodeControlPackets.mockReturnValueOnce({ controlPackets: [], rawData: new TextEncoder().encode("hello\n") });
    ...
    it("resolves pending command on matching counter", async () => {
        ...
        const result = await new Promise<{ output: string }>((resolve) => {
            ...
            decodePacket.mockReturnValueOnce({ ptype: "data" as const, counter: 7, srcMac: new Uint8Array(6), dstMac: new Uint8Array(6), sessionKey: 0 });
            decodeControlPackets.mockReturnValueOnce({ controlPackets: [], rawData: new TextEncoder().encode("result\n") });
            internals(client).handleMessage(Buffer.alloc(32), { address: "10.0.0.1", port: 20561 });
        });
    ...
    it("handles username request during connecting state", async () => {
        ...
        decodePacket.mockReturnValueOnce({ ptype: "data" as const, counter: 0, srcMac: new Uint8Array(6), dstMac: new Uint8Array(6), sessionKey: 0 });
        decodeControlPackets.mockReturnValueOnce({ controlPackets: [{ type: "username", data: new Uint8Array(0) }], rawData: new Uint8Array(0) });
        internals(client).handleMessage(Buffer.alloc(32), { address: "10.0.0.1", port: 20561 });
    ...
    it("handles end_auth during connecting → transitions to connected", async () => {
        ...
        internals(client).session = createSessionState({ sessionKey: 0x1234 });
        let resolved = false;
        internals(client).connectResolve = () => { resolved = true; };
        internals(client).connectReject = () => { };
        internals(client).connectTimer = setTimeout(() => { }, 99999);
        decodePacket.mockReturnValueOnce({ ptype: "data" as const, counter: 0, srcMac: new Uint8Array(6), dstMac: new Uint8Array(6), sessionKey: 0 });
        decodeControlPackets.mockReturnValueOnce({ controlPackets: [{ type: "end_auth", data: new Uint8Array(0) }], rawData: new Uint8Array(0) });
        internals(client).handleMessage(Buffer.alloc(32), { address: "10.0.0.1", port: 20561 });
    ...
    it("handles passsalt_client (EC-SRP key request) during connecting", async () => {
        ...
        (internals(client) as unknown as { clientECDH: unknown }).clientECDH = vi.mocked(crypto.createECDH)("prime256v1");
        (internals(client) as unknown as { clientPublicKey: Uint8Array }).clientPublicKey = new Uint8Array(65).fill(0x04);
        decodePacket.mockReturnValueOnce({ ptype: "data" as const, counter: 0, srcMac: new Uint8Array(6), dstMac: new Uint8Array(6), sessionKey: 0 });
        decodeControlPackets.mockReturnValueOnce({ controlPackets: [{ type: "passsalt_client", data: new Uint8Array(0) }], rawData: new Uint8Array(0) });
        internals(client).handleMessage(Buffer.alloc(32), { address: "10.0.0.1", port: 20561 });
        expect(mockSocket.send).toHaveBeenCalled();
    });
});
...
describe("MacTelnetClient AbortSignal", () => {
    it("destroys immediately if signal pre-aborted", async () => {
        ...
        controller.abort();
        const client = new MacTelnetClient({ targetMac: "aa:bb:cc:dd:ee:ff", username: "admin", password: "pw", signal: controller.signal });
        expect(internals(client as unknown as Awaited<ReturnType<typeof makeClient>>).state).toBe("disconnected");
    });
