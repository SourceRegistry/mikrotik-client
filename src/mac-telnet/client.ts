...
export class MacTelnetClient extends EventEmitter {
    ...
    async sendCommand(
        ...
        const promise = new Promise<ShellCommandResult>((resolve, reject) => {
            const timer = setTimeout(() => {
                ...
                reject(
                    new Error(`Command timeout after ${timeoutMs}ms: "${command}"`),
                );
            }, timeoutMs);
            ...
            this.socket!.send(
                ...
                (err) => {
                    if (err) reject(err);
                        ...
                        // Update byte counter
                        if (this.session) {
                            this.session = updateByteCounter(
                                this.session,
                                data.length,
                                "sent",
                            );
    ...
    private handleStartPacket(
        _packet: ReturnType<typeof decodePacket>,
        msg: Buffer,
    ): void {
    ...
    private processChallenge(
        ...
        if (this.authType === "md5" && control && control.data.length >= 16) {
            ...
            this.authType === "ec-srp" &&
            control &&
            control.data.length >= 65
        ) {
    ...
    private sendECSRPResponse(): void {
        ...
        responseData.set(
            ...
            usernameBytes.length + responseHash.length,
        );
