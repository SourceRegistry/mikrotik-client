...
describe('listCertificates', () => {
    ...
    it('maps certificate records to typed entries', async () => {
        const transport = createMockTransport();
        transport.print = vi.fn().mockResolvedValue([
            { '.id': '*1', fingerprint: 'AB:CD:EF', name: 'my-cert', 'remaining-days': '365', trusted: 'yes', 'key-usage': 'digitalSignature', kind: 'imported', 'valid-from': '2024-01-01 00:00:00', 'valid-to': '2025-01-01 00:00:00' },
    ...
    it('passes signal through to transport', async () => {
        ...
        expect(transport.print).toHaveBeenCalledWith('/certificate', expect.objectContaining({ signal: ctrl.signal }));
    });
});
