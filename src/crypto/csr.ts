...
function bigIntToBytes(value: bigint | number): Buffer {
    ...
    return Buffer.from(
        hex.padStart(hex.length + (hex.length % 2 ? 1 : 0), '0'),
        'hex',
    );
}
...
export function generateCsr(opts: GenerateCsrOptions): CsrResult {
    ...
    const rdns: Buffer[] = [];
    if (opts.countryName)
        rdns.push(derSet(derOid(OID_C), derPrintableString(opts.countryName)));
    if (opts.organizationName)
        rdns.push(
            derSet(derOid(OID_O), derUtf8String(opts.organizationName)),
        );
    ...
    const attributes =
        sans.length > 0 ? buildExtRequestAttr(sans) : derContext0(derSet());
...
function buildExtRequestAttr(sans: string[]): Buffer {
    ...
    return derContext0(
        derSequence(
            derSequence(derOid(OID_EXT_REQ), derSet(...ext)),
        ),
    );
}
