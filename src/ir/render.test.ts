...
import { type IRItem, type RouterOSConfig, type IRResourceBlock, type IRResourceCommand, createEmptyConfig } from './types';
import { parseExport } from './parse';

function resource(path: string, command: IRResourceCommand, properties: { name: string; value: string }[]): IRResourceBlock {
    return {
        kind: 'resource',
        path,
        command,
...
describe('renderScript', () => {
    describe('block rendering', () => {
        ...
        it('renders empty block', () => {
            const c = config([
                { kind: 'block', items: [{ kind: 'comment', text: '' }] },
            ]);
            const script = renderScript(c, { sortByDependency: false });
    ...
    describe('env rendering', () => {
        it('renders env set items', () => {
            const c = config([
                { kind: 'env', name: 'SNMP-_COMMUNITY', value: 'public' },
            ]);
            const script = renderScript(c, { sortByDependency: false });
        ...
        it('quotes env values with special chars', () => {
            const c = config([
                { kind: 'env', name: 'TEST', value: 'value with spaces' },
            ]);
            const script = renderScript(c, { sortByDependency: false });
        ...
        it('quotes values containing spaces', () => {
            const c = config([
                resource('/interface ethernet', 'set', [
                    { name: 'comment', value: 'my interface' },
                ]),
            ]);
    ...
    describe('round-trip (parse -> render -> parse)', () => {
      ...
      `;
            ...
            const originalResources = parsed.items.filter((item): item is IRResourceBlock => item.kind === 'resource');
            const reParsedResources = reParsed.items.filter((item): item is IRResourceBlock => item.kind === 'resource');
        ...
        /interface bridge add name=bridge1`;
            ...
            const rendered = renderScript(parsed);
            const reParsed = parseExport(rendered);
            const comments = reParsed.items.filter((item): item is { kind: 'comment'; text: string } => item.kind === 'comment');
