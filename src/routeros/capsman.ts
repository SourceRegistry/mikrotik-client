...
export const CAPS_MAN_WIRELESS_MODES: readonly CapsManWirelessMode[] = [
    "ap-bridge", "ap-bridge-network-aware", "ap-client", "client", "station",
    "station-list", "monitor", "tap", "tap5g", "nstd", "sniffer",
    "apk-bridge", "apk-client",
] as const;
...
export type CapsManHelpers = {
    configuration: {
        ...
        add(attributes: Record<string, RouterOSPrimitive>, options?: Omit<RouterOSCommandOptions, "attributes">): Promise<void>;
        set(id: string, attributes: Record<string, RouterOSPrimitive>, options?: Omit<RouterOSCommandOptions, "attributes">): Promise<void>;
        remove(id: string, options?: Omit<RouterOSCommandOptions, "attributes">): Promise<void>;
        ensure(attrs: { name: string; ssid?: string }, options?: Omit<RouterOSCommandOptions, "attributes">): Promise<void>;
    };
    datapath: {
...
function parseCapsManChannel(raw: RouterOSRecord): CapsManChannel {
    const id = raw[".id"];
    const name = raw["name"];
    const band = raw["band"] as "2ghz-b/g" | "2ghz-b/g/n" | "5ghz-a" | "5ghz-a/n" | "60ghz" | undefined;
    const controlWidth = raw["control-channel-width"] as "20mhz" | "40mhz" | "80mhz" | "auto-20-40mhz" | undefined;
...
function parseCapsManProvisioning(raw: RouterOSRecord): CapsManProvisioning {
    const id = raw[".id"];
    const name = raw["name"];
    const mode = raw["mode"] as "manual" | "none" | "profile" | "use-radio-data" | "use-ap-names" | undefined;
    ...
    return {
        ...
        ...(useDefaultOnFallback !== undefined && { "use-default-profile-on-fallback": parseBool(useDefaultOnFallback) }),
        ...(comment !== undefined && { comment }),
    };
}
...
function withId(id: string, attrs?: Record<string, RouterOSPrimitive>): Record<string, RouterOSPrimitive> {
    return { ".id": id, ...attrs };
}
...
export function createCapsManHelpers(client: DeviceTransport): CapsManHelpers {
    return {
        configuration: {
            list(options: RouterOSPrintOptions = {}): Promise<CapsManConfiguration[]> {
                return client.print("/caps-man/configuration", toPrintOptions(options)).then(r => r.map(parseCapsManConfiguration));
            },
            async add(attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/configuration/add", { ...options, attributes });
            },
            async set(id: string, attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/configuration/set", { ...options, attributes: withId(id, attributes) });
            },
            async remove(id: string, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/configuration/remove", { ...options, attributes: withId(id) });
            },
            async ensure(attrs: { name: string; ssid?: string }, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                const entities = await client.print("/caps-man/configuration", toPrintOptions({ queries: [`name=${attrs.name}`] }));
                ...
                if (existing?.[".id"]) {
                    ...
                    if (Object.keys(updates).length > 0) {
                        await client.execute("/caps-man/configuration/set", { ...options, attributes: withId(existing[".id"], updates) });
                    }
                } else {
        ...
        datapath: {
            list(options: RouterOSPrintOptions = {}): Promise<CapsManDatapath[]> {
                return client.print("/caps-man/datapath", toPrintOptions(options)).then(r => r.map(parseCapsManDatapath));
            },
            async add(attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/datapath/add", { ...options, attributes });
            },
            async set(id: string, attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/datapath/set", { ...options, attributes: withId(id, attributes) });
            },
            async remove(id: string, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
        ...
        security: {
            list(options: RouterOSPrintOptions = {}): Promise<CapsManSecurity[]> {
                return client.print("/caps-man/security", toPrintOptions(options)).then(r => r.map(parseCapsManSecurity));
            },
            async add(attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/security/add", { ...options, attributes });
            },
            async set(id: string, attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/security/set", { ...options, attributes: withId(id, attributes) });
            },
            async remove(id: string, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
        ...
        channel: {
            list(options: RouterOSPrintOptions = {}): Promise<CapsManChannel[]> {
                return client.print("/caps-man/channel", toPrintOptions(options)).then(r => r.map(parseCapsManChannel));
            },
            async add(attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/channel/add", { ...options, attributes });
            },
            async set(id: string, attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/channel/set", { ...options, attributes: withId(id, attributes) });
            },
            async remove(id: string, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
        ...
        provisioning: {
            list(options: RouterOSPrintOptions = {}): Promise<CapsManProvisioning[]> {
                return client.print("/caps-man/provisioning", toPrintOptions(options)).then(r => r.map(parseCapsManProvisioning));
            },
            async add(attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/provisioning/add", { ...options, attributes });
            },
            async set(id: string, attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/provisioning/set", { ...options, attributes: withId(id, attributes) });
            },
            async remove(id: string, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/provisioning/remove", { ...options, attributes: withId(id) });
            },
        },
        accessList: {
            list(options: RouterOSPrintOptions = {}): Promise<CapsManAccessList[]> {
                return client.print("/caps-man/access-list", toPrintOptions(options)).then(r => r.map(parseCapsManAccessList));
            },
            async add(attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/access-list/add", { ...options, attributes });
            },
            async set(id: string, attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/access-list/set", { ...options, attributes: withId(id, attributes) });
            },
            async remove(id: string, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/access-list/remove", { ...options, attributes: withId(id) });
            },
        },
        interface: {
            list(options: RouterOSPrintOptions = {}): Promise<CapsManInterface[]> {
                return client.print("/caps-man/interface", toPrintOptions(options)).then(r => r.map(parseCapsManInterface));
            },
            async add(attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/interface/add", { ...options, attributes });
            },
            async set(id: string, attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/interface/set", { ...options, attributes: withId(id, attributes) });
            },
            async remove(id: string, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
        ...
        manager: {
            list(options: RouterOSPrintOptions = {}): Promise<CapsManManagerInterface[]> {
                return client.print("/caps-man/manager", toPrintOptions(options)).then(r => r.map(parseCapsManManagerInterface));
            },
            async add(attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/manager/add", { ...options, attributes });
            },
            async set(id: string, attributes: Record<string, RouterOSPrimitive>, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
                await client.execute("/caps-man/manager/set", { ...options, attributes: withId(id, attributes) });
            },
            async remove(id: string, options: Omit<RouterOSCommandOptions, "attributes"> = {}) {
        ...
        registration: {
            list(options: RouterOSPrintOptions = {}): Promise<CapsManRegistration[]> {
                return client.print("/caps-man/registration", toPrintOptions(options)).then(r => r.map(parseCapsManRegistration));
            },
            watch: createWatch({
                transport: client,
