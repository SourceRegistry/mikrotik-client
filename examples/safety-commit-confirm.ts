...
import { commitConfirm, atomicScript, saveBackup, removeBackup, listBackups } from "../src/safety";
    ...
    try {
        ...
        const handle = await commitConfirm({
            ...
            ...(process.env.MIKROTIK_TIMEOUT_MS
                ? { timeoutMs: Number(process.env.MIKROTIK_TIMEOUT_MS) }
                : {}),
        });
...
/system note set comment="modified by atomicScript example"
      `,
        ...
        console.log("Set unsafeDirectApply: true to skip commit-confirm entirely.");
